...
public class HouseHostingPlugin extends Plugin {
    ...
    @Inject
    private HouseHostingConfig config;
    ...
    @Inject
    private HouseHostingOverlay houseHostingOverlay;
    ...
    @Provides
    HouseHostingConfig provideConfig(ConfigManager configManager) {
        return configManager.getConfig(HouseHostingConfig.class);
    }

    @Override
    protected void startUp() {
        overlayManager.add(houseHostingOverlay);
    ...
    protected void shutDown() {
        ...
        overlayManager.remove(houseHostingOverlay);
    ...
    public void onGameObjectSpawned(GameObjectSpawned event) {
        ...
        if (HouseHostingConstants.LIT_BURNER_IDS.contains(gameObject.getId())) {
            litBurners.put(gameObject, new BurnerState(client.getTickCount()));
            unlitBurners.remove(gameObject);
        } else if (HouseHostingConstants.UNLIT_BURNER_IDS.contains(gameObject.getId())) {
    ...
    public void onGameTick(GameTick tick) {
        ...
        if (client.getGameState() == GameState.LOGGED_IN && !litBurners.isEmpty()) {
            ...
            if (triggerPreWarningThisTick) {
                if (currentTick >= lastPreWarningTick + HouseHostingConstants.NOTIFICATION_COOLDOWN_TICKS) {
            ...
            if (triggerFinalAlarmThisTick && config.playFinalAlarm()) {
                if (currentTick >= lastFinalAlarmTick + HouseHostingConstants.NOTIFICATION_COOLDOWN_TICKS) {
                    playFinalAlarmSound();
    ...
    public void onChatMessage(ChatMessage event) {
        ...
        Matcher matcher = HouseHostingConstants.TIP_JAR_PATTERN.matcher(event.getMessage());
        if (!matcher.matches()) {
            return;
        }
    ...
    public void onGraphicChanged(GraphicChanged event) {
        ...
        final boolean isGenericLevelUpGraphic = player.hasSpotAnim(HouseHostingConstants.GENERIC_LEVEL_UP_GRAPHIC_ID);
        final boolean isLevel99Graphic = player.hasSpotAnim(HouseHostingConstants.LEVEL_99_GRAPHIC_ID);
    ...
    private boolean isGildedAltarOrExitPortalPresent() {
        for (int x = 0; x < HouseHostingConstants.SCENE_SIZE; x++) {
            for (int y = 0; y < HouseHostingConstants.SCENE_SIZE; y++) {
                for (int plane = 0; plane < HouseHostingConstants.MAX_PLANE; plane++) {
                    ...
                    for (GameObject gameObject : tile.getGameObjects()) {
                        if (gameObject != null && (HouseHostingConstants.GILDED_ALTAR_IDS.contains(gameObject.getId()) || gameObject.getId() == HouseHostingConstants.EXIT_PORTAL_ID)) {
                            return true;
                        }
                    }
                }
    ...
    private void playFinalAlarmSound() {
        try {
            audioPlayer.play(getClass(), HouseHostingConstants.FINAL_ALARM_SOUND_FILE, config.finalAlarmVolume());
        } catch (Exception e) {
    ...
    private void updateUnnotedMarrentillCount() {
        ...
        if (inventory != null) {
            for (Item item : inventory.getItems()) {
                if (item != null && item.getId() == HouseHostingConstants.CLEAN_MARRENTILL_ID && client.getItemDefinition(item.getId()).getNote() == -1) {
